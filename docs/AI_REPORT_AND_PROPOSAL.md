...
집행 단계 아래의 `실행 목표 비교`는 모델 예측이 아니다. 보수·목표·확대 중 선택한 방문객·관광소비 목표율을 최신 월 원자료에 단순 환산하여 `미실행`과 `실행 목표`를 비교하는 반응형 검토 도구다. 선택한 목표율은 Word 기획서에도 함께 전달한다.

```text
목표 방문자 수 = 최신 월 순 방문자 수 × (1 + 입력 방문 목표율)
추정 추가 관광소비액 = 최신 월 외지인 관광소비액 × 입력 소비 목표율
```

정식 다음 달 예측은 여러 시군구의 2~3년 공통 월간 데이터를 확보한 뒤, seasonal-naive baseline과 시간순 검증을 통과한 ML 모델로만 제공한다.
