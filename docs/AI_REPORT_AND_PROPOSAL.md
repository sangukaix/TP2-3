...
├─ proposal_presentation_v4.py   # 승인 레이아웃을 사용하는 최종 12장 PowerPoint 생성기
...
1. 지역명·기획안 제목 표지
2. 12장 구성에 맞춘 목차
3. 프로젝트 개요·기획 배경과 선택 지역 관광 Open API 대표 사진
4. 선택한 기획의 작동 방식·대상·시범 범위·필요 자원·최종 산출물
5. 향후 3~6개월 방문자·관광소비액 저장 ML 자연추세
6. 같은 기준의 유사 지역·공식 사례 비교와 선택 지역 적용 논리
7. 일정·담당·완료기준을 우상향 4단계 실행 여정으로 연결하고, 왼쪽 최종 산출물 레일에서 다음 운영 판단까지 제시
8. 방문자·소비·전환 등 검토 목표와 KPI 측정 기준
9. 견적 산출 기준과 8행 3열 편집형 표
10. 시범 종료 후 정례 운영·월별 개선·단계적 확장 체계
11. 관측 데이터·저장 ML·공식 사례/웹 근거·AI 실행 이력
12. 감사합니다 마무리

PowerPoint는 `python-pptx`로 생성하며 모든 텍스트·도형·표·차트는 편집 가능하게 유지한다. 최종 디자인은 사용자가 승인한 최신 디자인을 확장한 `tourism_strategy_12_slide_template.pptx`다. 완성된 글꼴·정렬·배치를 출력 때 다시 이동하거나 변경하지 않는다.

3장 사진은 선택 지역으로 조회한 한국관광공사 관광 Open API 결과 중 관광지·문화시설·축제·레포츠를 우선 사용한다. 이미지가 없거나 다운로드에 실패해도 PPT 생성은 계속된다. 6장은 보고서의 실제 비교 문장·선정된 공식 사례·지역 적용 조건을 표시한다. 같은 지역의 전년동월 방문 증감률이 있을 때만 전년동월을 100으로 둔 상대 지수를 표시하며, 수치가 없으면 차트를 제거한다. 차트는 보고서의 관측값과 저장 ML 결과만 사용하고 LLM이 새 수치를 만들지 않는다. 9장 견적은 성과 문구를 섞지 않고 산식·단가 근거와 8행 3열 편집형 표를 유지한다. 11장의 공급자 표시는 `agent_trace`가 있는 실행만 사용한다.
