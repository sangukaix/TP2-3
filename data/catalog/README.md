...
`data/raw/`는 제공기관 원본을 보관하는 읽기 전용 영역입니다. 팀 공유폴더에서 hash를 검증해 복제한 원본은 `data/source_snapshots/`에 별도 보관하며, 두 경로 모두 파일을 수정하지 않습니다. 카탈로그는 어느 지역 원본을 어떤 ML 어댑터로 읽을지와 출처 상태를 기록합니다.

## 새 지역 추가 순서

1. 수동 수집 원본은 `data/raw/<시도>/<시군구>/`, 공유폴더 snapshot은 `data/source_snapshots/<시도>/<시군구>/`에 원본 그대로 둡니다.
