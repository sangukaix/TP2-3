"""기존 파일을 덮어쓰지 않고 세 모델군을 하나의 joblib에 저장합니다.
...
def _factory(target: str, family: str):
    tree_targets = {"visitors", "navigation_searches", "lodging_searches"}
    if target not in tree_targets:
...
def train() -> dict:
    ...
    for family in ("current", "boosting", "xgboost"):
    ...
    artifact = {
        ...
        'models': models_by_family['current'],
        'models_by_family': models_by_family,
        'active_model_family': 'current',
    }
    metadata = {
        'version': artifact['version'],
        'region_code': '11680',
        'source_period': f"{frame['year_month'].iloc[0]}~{frame['year_month'].iloc[-1]}",
        'model_families': ('current', 'boosting', 'xgboost'),
        'active_model_family': 'current',
        # 기존 train_gangnam.py가 사용하는 호환 필드입니다.
        'evaluation': evaluation_by_family['current'],
