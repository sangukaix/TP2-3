...
def choose_targets() -> list[str]:
    ...
    # --all이면 7개 Target을 모두 학습합니다.
    if args.all:
        return list(TARGETS)

    # 아무 옵션이 없으면 방문자 수 하나만 먼저 학습합니다.
    return args.target or ["visitors"]


def load_data():
    """원본 ZIP을 읽고 학습용 월별 CSV를 준비합니다."""
    # 실제 데이터 파싱과 검증은 gangnam_data.py에 맡깁니다.
...
def load_previous_models() -> tuple[dict, dict]:
    ...
    # 이전 결과가 있으면 다른 Target의 모델을 보존합니다.
...
def prepare_missing_targets(data, models: dict, evaluations: dict) -> None:
    ...
    # 사이트는 7개 Target 구조를 기대하므로 미학습 Target도 기준선 결과를 넣습니다.
...
def save_for_server(data, models: dict, evaluations: dict) -> None:
    ...
    # joblib에는 모델을, JSON에는 모델 설명과 평가 결과를 저장합니다.
...
def main() -> None:
    ...
    # 2. 원본 데이터에서 학습용 표를 준비합니다.
    data = load_data()
    # 3. 기존 모델을 읽어 이미 학습한 Target을 유지합니다.
